...
from fsl.model import Transformer_Encoder, Focal_Loss, ProtoNet, TextCNN, CapsuleNet
...
class ModelManager():
    ...
    def init_model(self):
        if self.mode == 'train-test' or self.mode == 'cross validation':
            ...
            elif self.config.model == 'CapsuleNet':
                self.model = CapsuleNet.CapsuleNet(self.config)
            else:
    ...
    def __load_params(self, model, param_path):
        ...
        print('new_model_dict', len(new_model_dict))
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in new_model_dict}
        new_model_dict.update(pretrained_dict)
        print('new_model_dict after load', len(new_model_dict))
    ...
    def _caps_loss(self, y_true, y_pred, x, x_recon, lam_recon):
        """
        ...
        """
        L = y_true.to(torch.float32) * torch.clamp(0.9 - y_pred, min=0.) ** 2 + \
            0.5 * (1 - y_true) * torch.clamp(y_pred.to(torch.float32) - 0.1, min=0.) ** 2
        L_margin = L.sum(dim=1).mean()
        L_recon = nn.MSELoss()(x_recon.to(torch.float32), x.to(torch.float32))
        ...
        for epoch in range(self.config.epoch):
            for batch in train_dataloader:
                ...
                if self.config.model == 'Transformer Encoder':
                    ...
                    label_res = torch.zeros(label.size(0), 2).to(device=torch.device('cuda:0')).scatter_(1,
                                                                                                         label.view(-1,
                                                                                                                    1),
                                                                                                         1.)
                    logits, data_recon = self.model(data, label_res)
                    data_res = data.view(data_recon.size(0), data_recon.size(1), data_recon.size(2), data_recon.size(3))
                    train_loss = self._caps_loss(label_res, logits, data_res, data_recon,
                                                 lam_recon=0.0005 * 784) / data.size(0)
                    # print(train_loss)
                else:
                    logits = None
                    print('Mo Such Model')
            ...
            for batch in dataloader:
                ...
                if self.config.model == 'Transformer Encoder':
                    ...
                    label_res = torch.zeros(label.size(0), 2).to(device=torch.device('cuda:0')).scatter_(1,
                                                                                                     label.view(-1, 1),
                                                                                                     1.)
                    logits, data_recon = self.model(data)
                    data_res = data.view(data_recon.size(0), data_recon.size(1), data_recon.size(2), data_recon.size(3))
                    avg_test_loss += self._caps_loss(label_res, logits, data_res, data_recon,
                                                     lam_recon=0.0005 * 784).item() / data.size(0)
                else:
                    logits = None
                    print('Mo Such Model')
        ...
        for task_id in range(test_iteration):
            ...
            if self.config.adapt_iteration != 0:
                ...
                for j in range(self.config.adapt_iteration):
                    ...
                    support_acc, query_acc, metric, query_probs = self.model.fast_adapt(test_task,
                                                                   test_way,
                                                                   test_shot,
                                                                   test_query,
            ...
            with torch.no_grad():
                loss_sum, loss_query_CE, support_mi, query_mi, support_acc, query_acc, metric, query_probs = self.model.fast_adapt(test_task,
                                                                                                              test_way,
                                                                                                              test_shot,
                                                                                                              test_query,
                                                                                                              self.config.if_transductive,
                                                                                                              False)
            if metric[4] >= best_mcc or -metric[4] >= best_mcc:
                best_mcc = metric[4]
                best_performance = metric
                visual_prob = query_probs
            ...
            for task_id in range(1, self.config.meta_batch_size + 1):
                if self.config.dataset == "PPI":
                    ...
                    for i in range(self.config.train_iteration):
                        ...
                        loss_sum, loss_query_CE, support_mi, query_mi, support_acc, query_acc = self.model.fast_adapt_meta_train(
                            train_task,
                    ...
                    if loss_sum == 0 and loss_query_CE == 0 and support_mi == 0 and query_mi == 0 and support_acc == 0 and query_acc == 0:
                        flag = False
                    else:
                        self.optimizer.zero_grad()
                        loss_sum.backward()
